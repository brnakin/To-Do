...
from dotenv import load_dotenv  # Import dotenv for loading environment variables from .env files
import os  # Import os for accessing environment variables
from config import GENERATION_CONFIG, SAFETY_SETTINGS, PROMPT_PARTS  # Import configuration constants
...
def setup_generative_model():
    ...
    if not api_key:
        raise ValueError("GENAI_API_KEY is not set in the environment variables.")  # Error if API key is missing
...
def generate_content(model, prompt_parts):
    ...
    """
    response = model.generate_content(prompt_parts)  # Generate content using the model and prompt parts
    return response
...
def process_response(response_text):
    ...
    need_action = data_dict["need_action"].lower() == "yes"  # Determine if action is needed
    actions = None if data_dict["actions"].lower() == "null" else data_dict["actions"]  # Extract actions, handling null
    action_time = None if data_dict["action_time"].lower() == "null" else data_dict["action_time"]  # Extract action time, handling null
    advice = None if data_dict["advice"].lower() == "null" else data_dict["advice"]  # Extract advice, handling null

    if actions:
        actions = [action.strip() for action in actions.split("|")]  # Split and strip actions if not None
...
def util(prompt):
    ...
    response = generate_content(model, prompt_parts_extended)  # Generate content using the extended prompt parts
    ...
    return need_action, actions, action_time, advice, flag  # Return the structured information
